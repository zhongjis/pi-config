...
- Use isolation: "worktree" to run the agent in an isolated git worktree (safe parallel file modifications).`,
    ...
    async function ejectAgent(ctx, name, cfg) {
        ...
        if (cfg.allowDelegationTo?.length)
            fmFields.push(`allow_delegation_to: ${cfg.allowDelegationTo.join(", ")}`);
        if (cfg.disallowDelegationTo?.length)
            fmFields.push(`disallow_delegation_to: ${cfg.disallowDelegationTo.join(", ")}`);
