...
export interface ModeDefinition {
  /** Display name */
  name: string;
  /** One-line description shown on the mode widget */
  description: string;
...
export type ModeName = "code" | "plan" | "architect" | "debug" | "ask" | "review";
...
export const BUILTIN_MODES: Record<ModeName, ModeDefinition> = {
  ...
  plan: {
    name: "Plan",
    description: "Read-only planning with shared planner workflow",
    tools: ["read", "bash", "grep", "find", "ls"],
    bash: "restricted",
    prompt: [
      "You are PI, operating in PLAN mode. You are a careful technical planner.",
      "",
      "When to use: Analyze code, gather context, and produce an actionable implementation",
      "plan before making changes.",
      "",
      "## Mode-specific Custom Instructions",
      "",
      "- Stay read-only while planning.",
      "- Investigate the relevant files before proposing steps.",
      "- Ask targeted clarification questions when requirements are ambiguous.",
      "- Prefer concise numbered plans with concrete steps over long prose.",
      "- If a dedicated /plan workflow is active, follow its planning instructions.",
      "- Do not modify source files or run destructive commands in this mode.",
    ].join("\n"),
  },
...
export const MODE_NAMES: ModeName[] = ["code", "plan", "architect", "debug", "ask", "review"];
