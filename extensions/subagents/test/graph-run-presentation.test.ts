...
import { type FleetGraphRun, FleetList } from "../src/ui/fleet-list.js";
...
describe("workflow reports", () => {
  it("resolves the configured expand hint and preserves malformed entry data expanded", () => {
    ...
    const card = renderGraphRunEntryCard(malformed as Parameters<typeof renderGraphRunEntryCard>[0], theme, true);
    assert.ok(card);
  ...
  it("renders completed graph tool details as a structured tree", () => {
    ...
    expect(plain(renderGraphRunCard({ task, progress: task.graphRunProgress }, theme).render(120)).split("\n")).toEqual([
  ...
  it("collapses updates for counts and rejects a paused journal as a new resume target", () => {
    ...
    expect(stats(task.graphRunProgress)).toMatchObject({ total: 1, done: 1, running: false });
    expect(task.totalTokens).toBe(10);
    expect(pauseGraphRunTask(task, 200)).toBe(true);
    expect(elapsedMs(task, 1000)).toBe(100);
    expect(resolveResumeTarget(task.id, new Map([[task.id, task]]))).toMatchObject({ ok: false });
    expect(resumeGraphRunTask(task, 1000)).toBe(true);
...
describe("workflow disclosure states", () => {
  ...
  it("distinguishes queued work, child errors, and replayed work in the retained roster", () => {
    ...
    expect(plain(renderGraphRunCard({ task, progress: entries }, theme).render(120))).toContain("1 queued");
    task.status = "completed"; task.value = { result: "answer" };
    const report = plain(renderGraphRunCard({ task, progress: entries, expanded: true }, theme).render(120));
