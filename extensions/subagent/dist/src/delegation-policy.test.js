...
import { buildDelegationBlockedMessage, getCurrentDelegatorType, getPermittedDelegationTypes, hasDelegationPolicy, resolveDelegationRequest, } from "./delegation-policy.js";
describe("delegation-policy", () => {
    ...
    it("uses canonical type names for allowlists", () => {
        expect(getPermittedDelegationTypes({
            allowDelegationTo: ["ChengFeng", "TAISHANG", "missing"],
        }, availableTypes)).toEqual(["chengfeng", "taishang"]);
    });
    ...
    it("subtracts denylist from all available types when no allowlist is set", () => {
        ...
        }, availableTypes)).toEqual(["chengfeng", "wenchang", "guangguang", "taishang"]);
    });
    it("checks requested types case-insensitively", () => {
        expect(resolveDelegationRequest({
            allowDelegationTo: ["chengfeng", "taishang"],
        }, "TAISHANG", availableTypes)).toEqual({
            allowed: true,
    ...
    it("detects when a policy is absent", () => {
        ...
        expect(hasDelegationPolicy({ allowDelegationTo: ["chengfeng"] })).toBe(true);
    });
    it("reads the latest agent-mode session entry", () => {
        expect(getCurrentDelegatorType([
    ...
    it("formats a blocked delegation message with fallback info", () => {
        expect(buildDelegationBlockedMessage("fuxi", "unknown-agent", "general-purpose", ["chengfeng", "wenchang"])).toContain('"unknown-agent" (resolved to "general-purpose")');
    });
});
