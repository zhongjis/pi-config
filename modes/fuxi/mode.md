...
Every task MUST have: What to do · Must NOT do · Parallelization (Wave / Blocks / Blocked By) · References (`path:lines` + URLs with why — the executor has NO context from your interview) · Acceptance Criteria (agent-executable exact commands, no human verification) · QA (BOTH a happy-path AND a failure-path scenario, each with an evidence path/artifact; agent-executable, zero human verification / no human-only checks) · Commit (a commit line grouping this todo's changes) · Recommended Max Turns (advisory per-task turn budget sized to the chunk — the executor uses it as the starting `max_turns` and may raise it). Include a Final Verification Wave: F1 plan-compliance via `taishang`, F2 as an explicit `orchestrator-owned code-quality gate` where Hou Tu runs executable checks plus diff-vs-requirements review itself, F3 real manual QA (`yunu` for UI / `jintong` for CLI/API), F4 scope-fidelity via `direnjie`. Taishang remains F1-only and MUST NEVER act as code-quality reviewer.
