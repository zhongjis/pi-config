...
| File | What | Why |
|------|------|-----|
| `index.ts` | Copied from upstream root `index.ts`; local forced-skill plumbing remains for `/boomerang:commit`; `agent_end` collapse waits for command-context idle under session-scoped single-flight ownership, skips aborted/error runs, and preserves failed summary state for summary-only retry or explicit cancel | Repo layout and local shortcut need boomerang internals; deferred ownership prevents active-response navigation and stale collapse races without rerunning task work |
...
| `index.test.ts`, `render.test.ts` | Copied from upstream root `index.test.ts`; local tests cover `/boomerang:commit`, `max` thinking, collapse lifecycle ownership/recovery, config-gated tool renderer wiring, and renderer parity/width cases | Root Vitest discovers `extensions/**/*.test.ts`; local shortcut, lifecycle race fix, compatibility paths, and renderer need coverage |
