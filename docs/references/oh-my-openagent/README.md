...
`final-prompts/` contains 43 generated, final rendered prompts. `skills/ulw-plan/` contains the six raw upstream `ulw-plan` skill files copied byte-for-byte from the pinned source. Do not edit archive files by hand.
...
Both commands fetch `https://github.com/code-yeongyu/oh-my-openagent` at commit `14083b89f1cbf4680be13493a6c4afd67c957e8a` (`4.19.0`), install its locked dependencies with lifecycle scripts disabled, run the pinned source's internal prompt builders, and copy the raw `packages/shared-skills/skills/ulw-plan/` files. `sync` atomically replaces the archive targets. `check` regenerates in a temporary directory and compares without writing the archive.
