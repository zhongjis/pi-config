...
| `index.ts` | `session_start` early-returns when `!ctx.hasUI`; passes `ctx.ui.theme` into `retainThinkingStepsPatch` | Patch is render-only; avoids `ctx.ui` access without a UI. |
