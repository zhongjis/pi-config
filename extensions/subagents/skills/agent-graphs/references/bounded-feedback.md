...
- `evaluator` is a fixed `agent` template with `agent`, `prompt`, optional normal input ValueRefs, name and retry policy. `${feedback}` is reserved: it contains all prior iterations, current all-settled results and unresolved gaps. Do not provide `evaluator.input.feedback` or `evaluator.outputSchema`; the runtime owns them.
...
- Workflow snapshot IDs follow `^wf_[a-z0-9-]{6,}$` and must equal their filename stem. Symlinked `.pi`, checkpoint directories, snapshots and owner files are rejected. This is containment for persisted data, not a filesystem sandbox against concurrent same-user mutation.
