...
- Custom call rendering: cwd (shortened with `~`) and any timeout on the header line, with the bold command on its own indented `$` line below so a long worktree cwd never runs into the command after wrapping
