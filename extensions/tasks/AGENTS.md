...
| `src/tools/rendering.ts`, `src/tools/*.ts`, `test/tool-rendering.test.ts` | Task tools share compact TUI renderers: `renderCall` owns the header, collapsed `renderResult` emits short `keyword: content` summaries only (no trailing expand-hint line; the terminal line uses `└─` and ctrl+o still expands), expanded output returns raw `content`. | Preserves model-visible task tool output while keeping human-facing collapsed results glanceable. |
