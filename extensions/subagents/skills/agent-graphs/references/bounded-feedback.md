...
- The low-level `runGraph` API requires a synchronous `onCheckpoint(state, effectiveGraph)` writer for v2. The graph runtime supplies the filesystem writer and stable run ID. Throwing from the writer stops dispatch and retains the previous valid checkpoint.
...
- Graph run snapshot IDs follow `^agr_[a-z0-9-]{6,}$` and must equal their filename stem. Symlinked `.pi`, checkpoint directories, snapshots and owner files are rejected. This is containment for persisted data, not a filesystem sandbox against concurrent same-user mutation.
...
- Reload, session switch and shutdown preserve restorable active state. Explicit cancellation records terminal partial results; graph run completion deletes its checkpoint, and resume removes an already-terminal cancellation without relaunching it. Evaluator attempt numbers and remaining repair budgets survive interruption.
