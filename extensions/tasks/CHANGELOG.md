...
### Changed
- **Consolidated the six task tools into a single `Task` tool with an `op` discriminator (`create` / `update` / `list` / `get`) and batch payloads.** `create` accepts a `tasks[]` array and is all-or-nothing, returning the new IDs so dependencies can be wired in a follow-up `update`; `update` accepts a `tasks[]` array and is best-effort per item, reporting `Updated …` / `Rejected …` and surfacing a hard error only when every item is rejected. `list`/`get` output is byte-identical to the former `TaskList`/`TaskGet`. Schema is a flat `op`-discriminated object (StringEnum idiom, no root union) for cross-provider compatibility. Mode prompts, docs, and the `Task*` frontmatter wildcard are unchanged in intent. Covered by `test/task-tool.test.ts` and `test/tool-rendering.test.ts`.
- **Removed the dead `TaskOutput` / `TaskStop` process tools and their plumbing (`task-runner.ts`, `process-tracker.ts`, `BackgroundProcess`, process constants).** These operated only on processes registered via `ProcessTracker.track()`, which no producer called after `TaskExecute` was removed — so both tools always failed. The model no longer sees non-functional tools.
...
- **Collapsed tool renderers drop the trailing expand-hint line.** `renderSummaryLines` no longer appends `keyHint("app.tools.expand", …)`; the last `keyword: content` summary line now takes the terminal `└─` connector. Single-line results collapse to one line instead of two. ctrl+o still expands to raw `content` (the hint was cosmetic). Tasks-only change — diverges from the sibling extensions' shared hint convention by choice. Covered by `test/tool-rendering.test.ts`.
...
- **`agentType` parameter on `TaskCreate`** — opt-in field (e.g., `"chengfeng"`, or another available custom subagent type) that marks tasks for subagent execution.
