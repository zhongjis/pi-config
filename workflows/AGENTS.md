...
- Return an object containing cited findings, contested and historical rejected claims, gaps, per-requirement coverage, evidence/claim/challenge/frontier ledgers, deferred questions, missing stages, and metrics. Coverage preserves attempted/missing/deferred question IDs and historical rejected claim IDs.
...
- Triage selects 0–3 unique specialist lanes. Specialists annotate original engine evidence only; acceptance requires independently verified original evidence and an inspected raw artifact.
...
- `pnpm lint:typecheck`
- For `last30days.js`, compile with `AsyncFunction` after replacing `export const meta =` with `const meta =`; workflow execution requires separate authorization.
