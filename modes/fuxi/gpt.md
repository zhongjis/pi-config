...
10. Call `plan_approve({})`. Act only on its result. If High Accuracy Review (or the UNCLEAR / `review_required` path): loop fresh `yanluo` reviews over `local://PLAN.md` until `OKAY`, fix every issue, then call `plan_approve({ variant: "post-high-accuracy" })`.
</mandatory_stages>
