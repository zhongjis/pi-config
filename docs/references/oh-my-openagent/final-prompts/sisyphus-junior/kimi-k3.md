...
Every claim rests on tool output from this turn, not memory — verify once, then stop: a fact established this turn does not need a second pass. Note pre-existing issues without fixing them unless asked. Track completion with `todowrite`. No evidence means not complete.
