...
| `src/index.ts`, `src/agent-manager.ts`, `test/foreground-result-retrieval.test.ts`, `test/agent-identity-*.test.ts` | Foreground results expose canonical IDs; shared spawn allocates top-level name aliases; resume resolves live IDs/handles/names and rejects running/queued records before mutation | Keep result retrieval, steering and continuation on the original session while preserving mode policy and ownership |
