...
| `index.test.ts`, `render.test.ts` | Copied from upstream root `index.test.ts`; local tests cover `/boomerang:commit`, config-gated tool renderer wiring, and renderer parity/width cases | Root Vitest discovers `extensions/**/*.test.ts`; local shortcut and renderer need coverage |
