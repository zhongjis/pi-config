...
- Versioned `subagents:resume-target-v1` metadata maps the stable agent ID to its parent session, child session file, agent type, cwd, generation/revision, terminal state, and compatibility snapshot. Fresh execution begins at generation 0; each explicit resume begins `g+1/r0`. Compaction checkpoints, terminal state, and delivery flags advance revisions through one serialized store. Replay uses highest-generation/highest-revision ordering, and stale or concurrent writers cannot overwrite newer durable state.
...
- Execution and durability are distinct. Running V1 commits before provider entry, and a resumable success is not advertised until child JSONL plus terminal lookup metadata are durable. If terminal persistence fails after fresh output exists, the invocation returns `failed` with `persistence_failed`, retains that output for repair, blocks new provider entry until repair, and never starts or replays a replacement.
...
- Ownership is explicit: child JSONL owns conversation bytes; `AgentLifecycleStore` owns V1 generation/revision and delivery state; `AgentRun` owns live run projection; `external-contract-adapter.ts` owns terminal `subagents:record` compatibility plus completed/failed effects only. Created, started, steered, and other lifecycle emissions retain their existing runtime owners. Other components request durable transitions through these owners rather than constructing revisions directly.
- Barrier/effect order is running V1 → `subagents:started` → provider/tool execution → serialized compaction checkpoints → terminal V1 → terminal compatibility history → advisory completion/failure event. A rejected running barrier emits no started event. Current-process pending-terminal repair first reauthenticates the durable prefix and classifies the suffix, then commits only a matching recoverable candidate. Tool/RPC consumption commits `markConsumed` before acknowledgement. Background notification remains at-least-once: `sendMessage` runs outside the store queue, then `markNotified` commits. Send failure leaves `notified=false`; successful send followed by append failure may duplicate after restart. Compatibility repair appends history without replaying lifecycle events.
...
- Recovery follows a fixed matrix: durable terminal targets resume normally; replayed running targets with a clean terminal assistant suffix reconcile to terminal state without provider/tool/event replay; no suffix, user-only, metadata-only, pending-tool, mismatched pending result, or otherwise uncertain work commits conservative `unsafe_interrupted_operation`; missing, corrupt, incompatible, or scope-mismatched state keeps its typed failure and does not invent a newer valid snapshot. Current-process pending candidates use the same authenticated suffix classification before repair.
- Pi 0.79 defers a fresh session file's first flush until an assistant message. Before the pre-prompt durability barrier, the runtime writes current v3 session metadata (adding a non-context lifecycle anchor only when no leaf exists), then reopens the same `SessionManager`; normal Pi append semantics continue afterward.
- Rollback requires no schema migration. Builds that understand V1 can replay the highest valid generation/revision, ignore malformed later rows, and recover the last valid snapshot. No public V2 row, internal repair status, event payload, RPC field, or UI status is introduced.
...
- The original-race real-runtime regression controls a restored provider turn, emits a child compaction checkpoint, then releases terminal output. It asserts `g+1/r0 → r1 running checkpoint → r2 completed terminal`, fresh output, zero false failure, and exactly one resumed provider execution with zero tool use. Its fault variant fails the terminal append after `r1`, returns `persistence_failed`, preserves fresh output, and performs no duplicate work.
- Required gates are `pnpm test:extensions`, `pnpm test:integration`, and `pnpm lint:typecheck`; focused manual QA uses `pnpm exec vitest run --project integration test/integration/subagent-session-restoration.integration.test.ts test/integration/subagent-restart-durability.integration.test.ts --reporter=verbose`.
