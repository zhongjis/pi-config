...
function resolveWorkflowLayout(input: WorkflowDialogInput): { lines: WorkflowCardLine[]; detailOffset: number } {
  ...
  if (!inPhases && entry) {
    ...
    if (prompt.length === 0) {
      ...
      for (const text of shown) {
        for (const part of wrapTextWithAnsi(text, Math.max(1, rightWidth - 4))) {
          detailRows.push(detailBody(part, rightWidth));
        }
      }
  ...
  const detailOffset = Math.min(Math.max(0, state.detailOffset ?? 0), Math.max(0, detailRows.length - capacity));
  const rightRows = inPhases ? agentRows : detailRows.slice(detailOffset, detailOffset + capacity);
  lines.push(
    ...paneFrame({
  ...
  if (inPhases) {
    ...
    if (detailRows.length > capacity) hints.push(`pgup/pgdn details ${detailOffset + 1}–${Math.min(detailOffset + capacity, detailRows.length)}/${detailRows.length}`);
  ...
  for (const hint of wrapTextWithAnsi(` ${hints.join(" · ")}`, width)) {
    lines.push(clampLine([{ text: hint, color: "dim" }], width));
  }

  return { lines: lines.map(line => clampLine(line, Math.max(0, input.width ?? DEFAULT_WIDTH))), detailOffset };
}
...
export function handleWorkflowDialogKey(
  ...
  if (state.level === "agent" && (matchesKey(data, "pageDown") || matchesKey(data, "pageUp"))) {
    const delta = matchesKey(data, "pageDown") ? 5 : -5;
    return { state: { ...state, detailOffset: Math.max(0, (state.detailOffset ?? 0) + delta) } };
  }
  ...
  if (down || up) {
    ...
    return { state: { ...state, selectedAgent: clampIndex(view.clampedAgent + delta, view.visibleAgents.length), detailOffset: 0 } };
  }
  ...
  if (matchesKey(data, "enter") || matchesKey(data, "right")) {
    if (state.level === "phases") {
      ...
      return { state: { ...state, level: "agent", promptExpanded: false, detailOffset: 0 } };
    }
