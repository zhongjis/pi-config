...
export interface WorkflowSpawnResult {
  ...
  toolCalls?: number;
  /** Effective child working directory; gates run here. */
  cwd?: string;
}
...
export interface WorkflowHost {
  /** Release run-owned controllers after stopping and settling child runs. */
   ...
   *
   * `cwd` is the child's effective working directory. Optional for the same reason
...
export async function runWorkflow(options: RunWorkflowOptions): Promise<WorkflowRunResult> {
  ...
  function pauseGate(live: LiveAgent): Promise<void> {
    if (!paused || live.intent === "skip" || aborted || settled) return Promise.resolve();
    return new Promise<void>(resolve => {
      const wake = () => {
  ...
  return await new Promise<WorkflowRunResult>(resolve => {
    ...
    async function handleAgent(callId: number, payload: AgentCallPayload): Promise<void> {
      ...
      if (payload.isolation !== undefined) {
        respond(callId, false, undefined, "Workflow isolation is not supported: no isolation backend is installed.", true);
        return;
      }
      ...
      try {
        for (;;) {
          ...
          // Skipped while parked behind the limit: return its permit before
          // considering another pause hold, which would otherwise strand it.
          if (intent() === "skip") {
            semaphore.release();
            return settleSkipped({});
          }
          // Paused while parked behind the limit: hand back the permit and
          // wait at the gate, without starting another child.
          ...
          // Each progress row represents the logical call, including replaced attempts.
          if (result.tokens !== undefined) base.tokens = (base.tokens ?? 0) + result.tokens;
          if (result.toolCalls !== undefined) base.toolCalls = (base.toolCalls ?? 0) + result.toolCalls;
          ...
          if (intent() === "skip") result = { ...result, ok: false, skipped: true, error: "Skipped by user." };
          if (result.ok) {
