...
6. Use a conservative provider-supported subset for provider-facing structured-output schemas. Local schema validation NEVER proves provider acceptance. When a target provider rejects a semantic constraint such as array uniqueness via `uniqueItems`, enforce it in workflow body code and focused tests.
7. Evidence-producing prompts MUST request atomic, one-fact claims scoped to one assigned requirement and directly supported by cited excerpts. NEVER request report-shaped prose, compound claims, cross-requirement leakage, or unsupported synthesis.
...
| `key: string` | Stable identity for replay; lets a call's cached result be matched across runs regardless of position. Distinct from `label`; keep unique within a run. `pipeline`/`parallel` already auto-assign their children stable structural keys, so replay survives reordering for free; use `key` as the escape hatch for raw `Promise.all` or to override the auto key. |
...
1. Read `<run id>.workflow.jsonl` beside the persisted script for actual settled results before diagnosing unexpected/empty output; cached results are not necessarily nonempty. Diagnose asynchronous child/provider failures from retained child transcripts/details and inspector/progress, NEVER missing summary results alone; errors remain visible.
...
4. Replay re-executes script control flow and reuses journaled results by stable identity — a call's `key`, the structural key `pipeline`/`parallel` auto-assign, else its position — so a child repositioned by concurrency still matches its recorded result. Reuse remains a prefix in issue order: the first failed, changed, new, or missing call ends it, and that call plus every later one run live. Missing journals mean no cached prefix. Runs containing child-session `resume` calls decline journal replay.
