...
import { graphRunNodeArtifactId, readGraphRunNodeDetail } from "../src/graph/history-artifact.js";
...
function fixture() {
  ...
  const read = (index = 7) => readGraphRunNodeDetail({ cwd, sessionId }, runId, index);
...
describe("historical node artifacts", () => {
  it("aliases only run and validated history index with bounded path-safe names", () => {
    ...
    const alias = graphRunNodeArtifactId(hostile, 7);
    expect(alias).toMatch(/^[a-zA-Z0-9_-]{1,100}$/);
    expect(alias).toBe(graphRunNodeArtifactId(hostile, 7));
    expect(alias).not.toContain("private-binding");
    expect(alias).not.toBe(graphRunNodeArtifactId(hostile, 8));
    expect(alias).not.toBe(graphRunNodeArtifactId("another-run", 7));
    for (const index of [-1, 0.5, NaN, Infinity, Number.MAX_SAFE_INTEGER + 1]) expect(() => graphRunNodeArtifactId("run", index)).toThrow();
  });
  ...
  it("extracts only initial user and final assistant text with exact scope", () => {
    ...
    expect(readGraphRunNodeDetail({ cwd: f.cwd, sessionId: "other" }, f.runId, 7)).toBeUndefined();
    expect(readGraphRunNodeDetail({ cwd: f.cwd, sessionId: "../session" }, f.runId, 7)).toBeUndefined();
    expect(readGraphRunNodeDetail({ cwd: f.cwd + "-other", sessionId: f.sessionId }, f.runId, 7)).toBeUndefined();
    // encodeCwd collisions must not cross the exact-cwd boundary.
    expect(readGraphRunNodeDetail({ cwd: f.cwd.replaceAll("/", "-"), sessionId: f.sessionId }, f.runId, 7)).toBeUndefined();
    expect(readGraphRunNodeDetail(f, "other", 7)).toBeUndefined();
  });
  ...
  it("never creates missing artifact directories and fails safely on malformed or oversized data", () => {
    ...
    expect(readGraphRunNodeDetail({ cwd: f.cwd, sessionId: "missing" }, f.runId, 7)).toBeUndefined();
  ...
  it("resolves only selected history detail after metadata reload, outside the centered source", () => {
    ...
    const history = mergeGraphRuns([], decodeHistory(json).runs, resolver).get(f.runId);
    ...
    const run = { id: history.id, name: history.graphRunName, status: history.status, source, readHistoricalDetail: history.readNodeDetail };
