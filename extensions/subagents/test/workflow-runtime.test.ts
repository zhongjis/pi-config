...
import type { WorkflowJournalEntry } from "../src/graph/journal.js";
import { buildPhaseGroups, type WorkflowAgentEntry, type WorkflowEntry } from "../src/graph/progress.js";
import {
  assertBoundarySafe,
...
import { createWorkflowTask, updateWorkflowProgressBatch } from "../src/graph/task.js";
...
describe("the worker source itself", () => {
  it("parses as JavaScript", async () => {
    ...
    const { WORKER_SOURCE } = await import("../src/graph/worker-source.js");
  ...
  describe("effective configuration reported mid-run", () => {
    ...
    it("keeps a resumed row saying what the row it continues said", async () => {
      ...
      const host = {
        ...
        async resumeAgent(_id: string, _prompt: string, onResolved?: WorkflowSpawnRequest["onResolved"]) {
...
describe("Claude Code option compatibility", () => {
  ...
  it("accepts supported Claude Code agent options", async () => {
    const stub = stubHost();
    const result = await run(
      "return await agent('go', { label: 'L', phase: 'P', agentType: 'general-purpose', model: 'haiku', effort: 'high' });",
      { host: stub.host },
    );
...
describe("queued skips and retry accounting", () => {
  function deferred<T>() {
  ...
  it("skips a semaphore waiter while paused without requiring resume", async () => {
    const first = deferred<WorkflowSpawnResult>();
    const queued = deferred<void>();
    const started: string[] = [];
    let control: WorkflowControl | undefined;
    const abort = new AbortController();
    const deadline = setTimeout(() => abort.abort(), 2000);
    const done = run("return await parallel([() => agent('a'), () => agent('b')]);", {
      concurrency: 1,
      signal: abort.signal,
      onControl: value => { control = value; },
      onProgress: entries => {
        if (agentEntries(entries).some(entry => entry.index === 1)) queued.resolve();
      },
      host: {
        spawnAgent(request) { started.push(request.agentId); return first.promise; },
        abortAgent() { first.resolve({ ok: false, skipped: true }); },
      },
    });
    try {
      await Promise.race([queued.promise, done]);
      // The queued emit precedes pauseGate's microtask; let B reach semaphore.acquire().
      await Promise.resolve();
      expect(started).toEqual(["wf-agent-0"]);
      control?.pause();
      expect(control?.skip(1)).toBe(true);
      first.resolve({ ok: true, text: "first" });
      const result = await done;
      expect(result.status).toBe("completed");
      expect(result.value).toEqual(["first", null]);
      expect(control?.isPaused()).toBe(true);
      expect(started).toEqual(["wf-agent-0"]);
      expect(agentEntries(result.progress).filter(entry => entry.index === 1).at(-1)).toMatchObject({ state: "error", skipped: true });
    } finally {
  ...
  it.each(["complete", "skip-running", "skip-paused"] as const)("retains all attempt usage when the logical call ends with %s", async ending => {
    const firstStarted = deferred<void>();
    const secondStarted = deferred<void>();
    const retried = deferred<void>();
    const first = deferred<WorkflowSpawnResult>();
    const second = deferred<WorkflowSpawnResult>();
    let attempts = 0;
    let control: WorkflowControl | undefined;
    const task = createWorkflowTask({ id: "wf_accounting", script: HEAD });
    const abort = new AbortController();
    const deadline = setTimeout(() => abort.abort(), 2000);
    const done = run("const value = await agent('one'); return { value, spent: budget.spent() };", {
      signal: abort.signal,
      onControl: value => { control = value; },
      onProgress: entries => {
        updateWorkflowProgressBatch(task, entries);
        if (agentEntries(entries).some(entry => entry.attempt === 2)) retried.resolve();
      },
      host: {
        spawnAgent() {
          attempts++;
        ...
        abortAgent() {
          if (attempts === 1) first.resolve({ ok: false, skipped: true, tokens: 50, outputTokens: 10, toolCalls: 3 });
          else second.resolve({ ok: false, skipped: true, tokens: 20, outputTokens: 4, toolCalls: 2 });
        },
      },
    });
    try {
      await Promise.race([firstStarted.promise, done]);
      if (ending === "skip-paused") control?.pause();
      expect(control?.retry(0)).toBe(true);
      if (ending === "skip-paused") {
        await Promise.race([retried.promise, done]);
        expect(control?.skip(0)).toBe(true);
      } else {
        await Promise.race([secondStarted.promise, done]);
        expect(task.totalTokens).toBe(50);
        expect(task.totalToolCalls).toBe(3);
        if (ending === "skip-running") expect(control?.skip(0)).toBe(true);
        else second.resolve({ ok: true, text: "replacement", tokens: 20, outputTokens: 4, toolCalls: 2 });
      }
      const result = await done;
      const tokens = ending === "skip-paused" ? 50 : 70;
      const toolCalls = ending === "skip-paused" ? 3 : 5;
      expect(result.status).toBe("completed");
      expect(result.agentCount).toBe(1);
      expect(result.value).toEqual({ value: ending === "complete" ? "replacement" : null, spent: ending === "skip-paused" ? 10 : 14 });
      expect(agentEntries(result.progress).at(-1)).toMatchObject({ tokens, toolCalls, state: ending === "complete" ? "done" : "error" });
      expect(task.agentCount).toBe(1);
      expect(task.totalTokens).toBe(tokens);
      expect(task.totalToolCalls).toBe(toolCalls);
    } finally {
      clearTimeout(deadline);
      abort.abort();
      await done;
