...
- Parent verification reads changed files, executes focused tests, and performs applicable manual QA. Default/Gemini retain exact `lsp(operation:"diagnostics")` on changed code. [Hou Tu GPT](../../modes/houtu/gpt.md) uses diagnostics when available and applicable, limits `interactive_shell` to QA requiring interaction, and directly checks changed user-visible behavior with applicable browser, CLI, or API checks.
