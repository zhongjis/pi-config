...
Runs concurrently where independent; ALL must APPROVE. Ownership is fixed: F1 plan compliance audit (`taishang`); F2 `orchestrator-owned code-quality gate` run by Hou Tu; F3 parent manual QA run by Hou Tu (drive UI/browser through look_at / webapp-testing / agent-browser and CLI/API through bash/curl); F4 scope fidelity (`direnjie`). Approval marks that gate's Task completed and checks its PLAN row. Rejection leaves the gate `in_progress` and unchecked, repairs the responsible implementation workstream, then reruns every invalidated gate. Hou Tu surfaces all four approvals and waits for explicit user okay before declaring complete.
