...
The test framework is intentionally excluded from `install.sh` symlinking. The top-level allowlist symlinks only runtime support items (`agents/`, `modes/`, `caveman.json`, `pi-herdr-btw.json`, `session-summary.json`, `subagents.json`, `tool_models.json`, `scripts/`, `themes/`, and `workflows/`) into `~/.pi/agent/`. Test infrastructure (`test/`, `vitest.config.ts`, `package.json`, `node_modules/`, etc.) stays in the repo only.
