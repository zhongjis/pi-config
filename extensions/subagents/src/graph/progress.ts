/**
 * progress.ts — the graph run progress model, ported from Claude Code.
 *
 ...
 * the inline card, the graph runs dialog, the fleet widget — derives its shape
...
export function stats(progress: readonly GraphRunEntry[], agentCount = 0): GraphRunStats {
  ...
  for (const entry of collapse(progress).agents) {
    seen++;
    if (entry.state === "done") {
      done++;
      started++;
...
export function elapsedMs(
  task: { startTime: number; endTime?: number; totalPausedMs?: number; pausedAt?: number },
  now: number,
): number {
  return Math.max(0, (task.endTime ?? task.pausedAt ?? now) - task.startTime - (task.totalPausedMs ?? 0));
}
