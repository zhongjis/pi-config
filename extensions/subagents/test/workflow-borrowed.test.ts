...
describe("gate", () => {
  ...
  it("runs gates in the child's effective cwd", async () => {
    const { host, gateCalls } = stubHost({ reply: () => ({ ok: true, text: "done", cwd: "/tmp/child-cwd" }) });
    await run('return await agent("x", { gate: "npm test" });', { host });
    expect(gateCalls).toEqual([{ command: "npm test", agentId: "wf-agent-0", cwd: "/tmp/child-cwd" }]);
  });

  it.each(['"worktree"', '"none"', 'null', 'false', 'undefined'])("rejects isolation %s before spawning", async value => {
    const { host, calls, gateCalls } = stubHost();
    const result = await run(`return await agent("x", { isolation: ${value}, gate: "npm test" });`, { host });
    expect(result.status).toBe("failed");
    expect(result.error).toContain("no isolation backend");
  ...
  describe("guardrails", () => {
    const rejected: [string, string, string][] = [
      ...
      [
        ...
        "agent() opts.isolation is not supported",
      ],
      [
        "effort",
