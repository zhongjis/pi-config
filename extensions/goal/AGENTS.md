...
| `src/goal/render.ts` (local-only file), `src/index.ts` | Added shared width-safe `renderCall`/`renderResult` pairs to all three goal tools with goal-local objective/status/budget parsing and exact raw expansion | Not in upstream; compact TUI must preserve `result.content` and omit zero/unavailable telemetry. Re-apply on sync: keep `render.ts` and re-add both renderer props to each `pi.registerTool(...)` in `src/index.ts` |
| `src/goal/ui.ts` | `updateGoalUi` publishes the current goal indicator on the `Symbol.for("pi-goal:footer")` global bridge (`{ getIndicator(isIdle) }`) and skips installing its own footer when the `qol` extension owns the footer slot (`Symbol.for("pi-visuals:footer")`) | Only one `ctx.ui.setFooter()` slot exists; upstream's unconditional `setFooter` clobbered this repo's `qol` footer when a goal was set/cleared. Bridge and standalone fallback share presentation-only, idle-aware elapsed materialization; same-ID observations merge a monotonic high-water without mutating persisted goal state. Diverges from upstream; re-apply on sync |
| `test/footer-bridge.test.ts` | New repo-local test (not vendored) covering the `qol` footer-bridge integration above | Verifies footer ownership/delegation, busy elapsed advancement, idle freeze, stale/new persisted baselines, new-ID reset, null/inactive states, and standalone parity. Keep on sync |
...
- On re-sync, re-apply the scope migration, the bun→vitest test migration, the `extensions/pi-goal`→`extensions/goal` storage-path rename, and the `qol` footer-bridge integration in `src/goal/ui.ts` above, then run the root `unit` vitest project.
