...
| `src/agent-manager.ts`, `src/agent-run.ts`, `src/tools/agent.ts`, `test/{agent-manager,agent-run}.test.ts` | AgentRun lifecycle publication plus store-backed fresh/resume begin and terminal barriers; failed terminal commits retain a repair overlay and block provider re-entry until repair | Keeps runtime effects behind durable metadata without changing public invocation/status unions or external event names |
