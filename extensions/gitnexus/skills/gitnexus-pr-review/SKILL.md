...
> If "Index is stale" → run `/gitnexus analyze` in pi before reviewing.
...
| --- | --- |
| **Correctness** | `gitnexus_context` shows callers — are they all compatible with the change? |
| **Blast radius** | `gitnexus_impact` shows d=1/d=2/d=3 dependents — anything missed? |
| **Completeness** | `gitnexus_detect_changes` shows all affected flows — are they all handled? |
| **Test coverage** | `gitnexus_impact({includeTests: true})` shows which tests touch changed code |
