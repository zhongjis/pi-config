...
- Put extension README rules in `docs/specs/extensions.md` and testing rules in `docs/specs/testing/`.
...
- Treat `decisions/` as append-only ADRs: one decision per document, including why the chosen option won.
- Keep `references/` for stable, citable external material only.

## Work Guidance

- Update docs near the contract they describe.
- For extension testing changes, update `docs/specs/testing/README.md` or its child docs instead of root README.
...
| `specs/` | this file | Panda Harness contracts, including testing conventions. |
| `decisions/` | this file | Append-only, single-decision ADRs. |
