...
- `skills/VERSION` — single-line binary version that the vendored skills track; currently `1.6.6`. Updated only by `scripts/sync-gitnexus-resources.sh`. Drift vs installed binary produces a non-blocking `notify('warning')` at `session_start`.
