...
| `extension_tools` | all available extension tools | Extension-tool allowlist after extensions are available. Exact names are supported; entries ending in `_*` match that tool-name prefix (for example `codegraph_*`). `none` means no extension tools. Cannot grant built-ins. |
...
| `extensions` | `extension_tools` omitted | `extension_tools: none` | `extension_tools: foo` | `extension_tools: codegraph_*` |
|--------------|---------------------------|-------------------------|------------------------|-------------------------------|
| omitted/`true` | all available extension tools | no extension tools | only exact `foo` | tools whose names start with `codegraph_` |
| CSV | all available extension tools (source names preserved where supported) | no extension tools | only exact `foo` | tools whose names start with `codegraph_` |
