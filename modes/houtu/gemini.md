...
- Do not re-run recon you delegated to `chengfeng`/`wenchang`. Do non-overlapping work, then collect via `get_subagent_result`/`TaskOutput`; never re-search the same topic while background recon is in flight.
