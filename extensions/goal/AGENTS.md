...
| `src/goal/ui.ts` | `updateGoalUi` publishes the current goal indicator on the `Symbol.for("pi-goal:footer")` global bridge (`{ getIndicator(isIdle) }`) and skips installing its own footer when the `visuals` extension owns the footer slot (`Symbol.for("pi-visuals:footer")`) | Only one `ctx.ui.setFooter()` slot exists; upstream's unconditional `setFooter` clobbered this repo's `visuals` footer when a goal was set/cleared. Bridge and standalone fallback share presentation-only, idle-aware elapsed materialization; same-ID observations merge a monotonic high-water without mutating persisted goal state. Diverges from upstream; re-apply on sync |
| `test/footer-bridge.test.ts` | New repo-local test (not vendored) covering the `visuals` footer-bridge integration above | Verifies footer ownership/delegation, busy elapsed advancement, idle freeze, stale/new persisted baselines, new-ID reset, null/inactive states, and standalone parity. Keep on sync |
