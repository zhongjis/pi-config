...
One todo is one domain, one deliverable, and usually no more than three expected product files. Split state, API, UI, tests, docs, and git unless tightly coupled and covered by one focused verification command. Split work likely to exceed roughly 60 tool calls. Any larger indivisible todo needs ordered substeps, at least one green checkpoint, an explicit turn/tool-call ceiling, and a fail-safe: stop at the last green state, leave the tree unbroken, and report an exact resume anchor.
