/**
 * Agent Modes - switch between code, plan, architect, debug, ask, and review modes.
 *
...
type GlobalScope = typeof globalThis & {
  __piAgentModesApi?: AgentModesApi;
};
...
export default function agentModes(pi: ExtensionAPI) {
  ...
  pi.registerFlag("agent-mode", {
    description: "Start in a specific agent mode (code, plan, architect, debug, ask, review)",
    type: "string",
  });
  ...
  function buildRestrictionSummary(ctx: ExtensionContext, mode: ModeDefinition): string {
  ...
  function renderModeTab(ctx: ExtensionContext, name: ModeName): string {
    const t = ctx.ui.theme;
    const label = ` ${modes[name].name.toUpperCase()} `;
    if (name === activeMode) {
      return t.bg("selectedBg", t.fg("accent", t.bold(label)));
    }
  ...
  function updateStatus(ctx: ExtensionContext): void {
    if (activeMode === "off") {
      ctx.ui.setWidget("agent-mode-tabs", undefined);
      return;
    }
    updateModeTabs(ctx, modes[activeMode]);
  }
    ...
    globalScope.__piAgentModesApi = {
      ...
      async setMode(mode: AgentModeState, options?: { notify?: boolean }): Promise<boolean> {
        ...
        if (mode === "off") {
          await turnOff(ctx);
          persistState();
          if (options?.notify) ctx.ui.notify("Agent modes disabled", "info");
          return true;
        }
  ...
  async function cycleMode(ctx: ExtensionContext): Promise<void> {
    const cycle: AgentModeState[] = [...MODE_NAMES, "off"];
  ...
  pi.registerCommand("agent-mode", {
    description: "Switch agent mode (code, plan, architect, debug, ask, review) or run setup",
    ...
    handler: async (args, ctx) => {
      ...
      if (arg) {
        ...
        ctx.ui.notify(`Switched to ${modes[arg].name} mode`, "info");
        persistState();
        return;
      }
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    if (typeof flagValue === "string" && flagValue) {
      const name = flagValue.toLowerCase();
      if (isModeName(name)) {
        await applyMode(name, ctx);
    ...
    const stateEntry = entries
      ...
      .pop() as { data?: { mode: AgentModeState } } | undefined;

    if (stateEntry?.data?.mode === "off") {
      await turnOff(ctx);
    } else if (stateEntry?.data?.mode && isModeName(stateEntry.data.mode)) {
