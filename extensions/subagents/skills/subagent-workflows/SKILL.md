...
description: Author and modify SubagentWorkflow scripts; API, orchestration patterns, and resume reference. Requires separate explicit user opt-in to execute workflows.
---
...
- The user invoked a skill or slash command explicitly requesting workflow execution. Reading or invoking this authoring skill alone MUST NOT grant workflow execution permission.
...
- **Migrate** — discover sites → transform disjoint files → verify
...
Every script must begin with `export const meta = {...}`:
  export const meta = {
    ...
    description: 'Find flaky tests and propose fixes',   // one-line, shown in the workflow card
...
Script body hooks:
- agent(prompt: string, opts?: {label?: string, phase?: string, schema?: object, model?: string, effort?: string, agentType?: string, gate?: string, resume?: string}): Promise<any> — spawn a subagent. Without schema, returns its final text as a string. With schema (a JSON Schema), the subagent is given a StructuredOutput tool built from it and agent() returns the validated object — no parsing needed. A payload that does not match is rejected back to the child, which corrects it; a child that never answers through the tool gets one more prompt and then fails, so the call returns null — filter after every schema stage. Returns null if the user skips the agent mid-run or the subagent dies on a terminal API error after retries (filter with .filter(Boolean)). opts.label overrides the display label. opts.phase explicitly assigns this agent to a progress group (use this inside pipeline()/parallel() stages to avoid races on the global phase() state — same phase string → same group box). opts.model selects the model only when the agent definition has no model chain; configured model chains remain authoritative, including :fast policy. Default to omitting it — an agent without a configured model inherits the main-loop model (the resolved session model), which is almost always correct. Only set it when you're highly confident a different tier fits the task; when unsure, omit. opts.effort overrides the reasoning effort for this agent call ('minimal' | 'low' | 'medium' | 'high' | 'xhigh' | 'max') — omit to use the agent definition's own level, then the SDK selected-model default (never parent thinking); use 'low' for cheap mechanical stages and higher tiers only for the hardest verify/judge stages. opts.gate: '<command>' runs a shell command after the agent finishes and requires it to pass — a non-zero exit marks the agent failed and the command's output becomes the error; prefer gate: 'npm test' over asking another agent whether the code looks right. opts.resume: '<label>' continues the child that ran under that label instead of starting fresh, so an iterative loop keeps its context — it cannot be combined with agentType, model, effort, gate or schema. opts.agentType uses a custom subagent type instead of general-purpose — resolved from the same registry as the Agent tool; composes with schema. Consult the current Agent tool description for the available agent types and their configuration.
