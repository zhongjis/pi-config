...
- Required `agent()` failures MUST reject; `{optional:true}` permits only ordinary terminal child/schema/gate failures to return null. Configuration, policy, and programming errors MUST still reject. User skips remain null; `pipeline()` MUST stop remaining item stages on null, NEVER other falsy values. Orchestration MUST propagate thrown errors.
- Explicit `outcome.succeed/partial/fail` MUST separate objective outcome from execution status; plain returns remain undeclared. Reserved `$subagentWorkflowOutcome` envelopes MUST validate before root normalization or nested propagation. Persisted outcome is optional for legacy records; malformed snapshots retain raw fallback. Helpers/envelopes MUST be shallow-frozen without freezing user values.
- Completion notifications/reports MUST disclose declared outcome, execution, and child settlement independently, prioritizing execution failure; delivery timing/channels remain unchanged.
