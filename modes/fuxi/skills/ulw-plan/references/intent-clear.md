...
Run the durable approval gate (mechanics in `full-workflow.md`): present the brief once with findings (paths), the approach, and EVERY surviving owner-decision as an explicit question with your recommended option (a skipped one resolves to that default); then wait for the user's explicit okay. If final approval would be your ONLY question, you have defaulted forks you should have surfaced - list them first. After approval: call `plan_scaffold`, run mandatory `direnjie`, APPEND the todos, fill the human TL;DR last. Then either run the dual `yanluo` + independent `taishang` high-accuracy review if `review_required: true`, or present the summary through `plan_approve`. Never pick review for the user when review was not requested; never begin execution. Only the user may start Hou Tu through `/handoff:start-work`.
</approval_and_deliver>
...
4. Approval brief -> explicit okay -> `plan_scaffold` -> append todos -> if `review_required`, run dual review and deliver receipts; otherwise deliver through `plan_approve`.
</worked_example>
