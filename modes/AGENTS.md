...
- Hou Tu executes `local://PLAN.md` as a per-task pi-task DAG, not per-wave: `TaskCreate` one pi-task per top-level plan task (agentType from the plan `Agent:` field, 6-section contract in `description`), wire deps with `TaskUpdate addBlockedBy`, run via `TaskExecute` (never raw `Agent()` for plan work; `additional_context` is batch-shared so per-task context stays in `description`). This couples Hou Tu to the `tasks` extension: a pi-task `completed` means the agent stopped (self-report OR interrupted), NOT verified — only a `PLAN.md` checkbox flips after Hou Tu's evidence. Retries re-run fresh via `TaskExecute` (re-open with `TaskUpdate status:pending`), never `Agent(resume)`. `autoCascade` stays OFF so Hou Tu is the QA gate between tasks. The houtu invariant "One TaskExecute launch = one bounded plan task" is asserted in `test/fuxi-clearance.test.ts`.
