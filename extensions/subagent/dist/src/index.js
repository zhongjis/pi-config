...
import { BACKGROUND_STALE_STEER_AFTER_MS, BACKGROUND_SUPERVISION_INTERVAL_MS, getBackgroundSupervisionAction, getLastProgressAt, } from "./background-supervision.js";
...
export default function (pi) {
    ...
    function sendStaleAgentReminder(record, idleMs, action) {
    ...
    const turnAbortSignals = new WeakSet();
    function getAbortSignal(ctx) {
        return ctx.signal;
    }
    function bindTurnAbortSignal(signal) {
        if (!signal || turnAbortSignals.has(signal))
            return;
        const onAbort = () => {
    ...
    function requireSpawnableType(type) {
        reloadCustomAgents();
        const resolved = resolveType(type);
        if (!resolved || !isValidType(resolved))
            throw new Error(formatUnavailableAgentType(type));
        return resolved;
    }
    ...
    function syncSessionContext(ctx) {
        currentCtx = ctx;
        if (!ctx?.hasUI)
            return;
    ...
    function getResolvedModelLabel(model) {
        if (!model?.id)
            return undefined;
    ...
    async function waitForAgentCompletionWithSupervision(record, signal) {
...
- Use isolation: "worktree" to run the agent in an isolated git worktree (safe parallel file modifications).`,
        ...
        execute: async (toolCallId, params, signal, onUpdate, ctx) => {
            ...
            const currentDelegatorType = getCurrentDelegatorType(ctx.sessionManager.getEntries());
            if (currentDelegatorType) {
                ...
                if (delegatorConfig && hasDelegationPolicy(delegatorConfig)) {
                    ...
                    if (!delegation.allowed) {
                        return textResult(buildDelegationBlockedMessage(currentDelegatorType, rawType, delegation.requestedType, delegation.permittedTypes));
                    }
                }
            }
            ...
            if (params.resume) {
                ...
                return textResult(getRecoveredResultText(record), buildDetails(detailBase, record));
            }
            ...
            if (runInBackground) {
                ...
                return textResult(`Agent ${isQueued ? "queued" : "started"} in background.\n` +
                    ...
                    `Do not duplicate this agent's work or leave it unattended for long.`, { ...detailBase, toolUses: 0, tokens: "", durationMs: 0, status: "background", agentId: id });
            }
            ...
            return textResult(`Agent completed in ${formatMs(durationMs)} (${statsParts.join(", ")})${getStatusNote(record.status)}.\n\n` +
                getRecoveredResultText(record), details);
        },
    });
    ...
    pi.registerTool({
        ...
        description: "Check status and retrieve results from a background agent. Use it to actively supervise long-running work started by Agent.",
        parameters: Type.Object({
