...
- Parent verification reads every changed file, reviews the full applicable diff, and inspects actual command/scope/output/exit status under [shared verification ownership](../../modes/AGENTS.md). Workers own focused regression and file-local lint/format; parent owns final executable integration for combined changes after relevant writers finish. Valid evidence, including diagnostics and parent QA, is reusable; missing, invalidated, diagnostic, or explicitly required checks run without delegation/phase-only repetition. Default/Gemini retain `lsp(operation:"diagnostics")` evidence on changed code; GPT uses diagnostics when available and applicable and limits `interactive_shell` to QA requiring interaction. Parent QA covers changed user-visible surfaces and affected interactions.
