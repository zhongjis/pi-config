...
上游本身镜像的 pi 文档快照见上文「来源」：`earendil-works/pi` @ `f429ddb`（与本 vendoring commit 不同，前者是 pi 文档，后者是 playbook repo）。
The upstream pi docs snapshot it mirrors is `earendil-works/pi` @ `f429ddb` (see 来源 above) — distinct from the playbook repo commit `2f93257`.
