...
function readSessionProfile(ctx: ExtensionContext): string | undefined {
  ...
  for (let i = entries.length - 1; i >= 0; i -= 1) {
    ...
    if (
      entry?.type !== "custom" ||
      entry.customType !== PROFILE_STATE_CUSTOM_TYPE
    )
      continue;
...
function writeSessionProfile(pi: ExtensionAPI, name: string): void {
  (pi as ProfilesExtensionAPI).appendEntry?.(PROFILE_STATE_CUSTOM_TYPE, {
    name,
  });
}
...
export function installModelRegistryFilter(registry: ModelRegistryLike): void {
  ...
  registry.getAvailable = function getProfileAvailable(
    this: ModelRegistryLike,
  ): ModelLike[] {
    const available = policy.originalGetAvailable?.call(this) ?? [];
    if (policy.activeConfigs.size === 0) return available;
    const allowed = unionAllowedProviders(
      Array.from(policy.activeConfigs.values()),
    );
...
async function setModel(
  ...
  if (ok === false) {
    ...
    notify?.(
      `Profiles: model unavailable: ${typed.provider ?? "unknown"}/${typed.id ?? "unknown"}`,
      "error",
    );
    return false;
  }
...
export async function forceProfileModel(
  ...
  const target =
    findConfigModel(config.defaultModel, ctx.modelRegistry) ??
    findFirstAllowedModel(ctx.modelRegistry, config);
  if (!target) {
    notify?.(
      `Profiles: no model available for providers: ${config.providers.join(", ")}`,
      "error",
    );
    return;
  }
...
export default function profilesExtension(pi: ExtensionAPI): void {
    ...
    if (!cliFlagConsumed) {
      ...
      if (typeof flagValue === "string" && flagValue.trim()) {
        ...
        notifyOnce(
          ctx,
          `cli-unknown:${name}`,
          `Profiles: --profile "${name}" is unknown; falling back.`,
          "warning",
        );
      }
    }
  ...
  function getActiveConfig(): ProfileConfig {
    return (
      ...
      DEFAULT_PROFILES_CONFIG.profiles[activeName] ?? {
        providers: [],
  ...
  function updateStatus(ctx: ExtensionContext): void {
    ...
    ctx.ui.setStatus(
      "profile",
      show ? `profile: ${active.statusText}` : undefined,
    );
  }
  ...
  async function applyProfile(
    ...
    if (options.restoreSession) {
      ...
      if (
        ...
        flagValue.trim() &&
        config.profiles[flagValue.trim()]
      ) {
    ...
    updateStatus(ctx);
    await forceProfileModel(pi, ctx, active, (message, type) =>
      notifyOnce(ctx, `model:${message}`, message, type),
    );
  }
  ...
  pi.registerFlag("profile", {
    description:
      "Set the provider profile for this session (default/opencode/local/...).",
    type: "string",
  });
  ...
  pi.on("tool_call", async (event, ctx) => {
    ...
    const requestedType =
      typeof event.input.subagent_type === "string"
        ...
        : "";
    if (
      active.blockedAgents?.map(normalize).includes(normalize(requestedType))
    ) {
      return {
        block: true,
