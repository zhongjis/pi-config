...
| `index.ts` | Non-status tool calls short-circuit without spawning when no valid marker exists. Marker-root tool calls first run a normal `tools/call` (`serve`); if the marker-root call reports uninitialized, it runs shared `initCodeGraphProject` inside the same-root queue and retries the original call once (`serve -> init -> serve`). `codegraph_status` stays inspect-only: no-marker returns no-marker text without spawning, ready marker roots return MCP status, cold marker roots return cold-enabled text, and status never runs init. | Enables bare-marker first-query bootstrapping without status-side mutation, while avoiding subprocess churn and surprise init for projects that lack a marker. |
| `index.ts` | Tool registrations define compact `renderCall`/`renderResult` TUI renderers while preserving `execute` content text exactly. Collapsed results summarize status, counts, highlights, and expand key hint; expanded results include raw text. | Keeps CodeGraph answers model/session-visible unchanged while improving default collapsed TUI UX and respecting Pi/user tool expansion state. |
| `test/index.test.ts` | Focused coverage for CodeGraph tool TUI renderers, collapsed/expanded behavior, partial/error states, and unchanged `content[0].text`. | Locks local TUI behavior without changing CodeGraph MCP semantics. |
...
- `codegraph` CLI is assumed on `PATH`; CodeGraph queries spawn it per request only for marker-enabled projects. No-marker tools return without spawning.
