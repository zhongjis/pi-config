...
function assertString(value: unknown, name: string): string {
	if (typeof value !== "string" || !value.trim()) throw new Error(`look_at ${name} must be a non-empty string.`);
	return value.trim();
}

function normalizeParams(params: unknown): NormalizedLookAtParams {
	if (!isRecord(params)) throw new Error("look_at parameters must be an object.");
	const filePath = typeof params.file_path === "string" && params.file_path.trim() ? params.file_path.trim() : undefined;
	const imageData = typeof params.image_data === "string" && params.image_data.trim() ? params.image_data.trim() : undefined;
	...
	return {
		...
		mime_type: typeof params.mime_type === "string" && params.mime_type.trim() ? params.mime_type.trim() : undefined,
	};
}
...
function assertSupportedMime(mimeType: string): string {
	...
	if (!SUPPORTED_MIME_TYPES.has(normalized)) {
		throw new Error(`look_at unsupported mime_type "${mimeType}". Supported: ${Array.from(SUPPORTED_MIME_TYPES).join(", ")}.`);
	}
	return normalized;
}
...
function assertByteLimit(buffer: Buffer): void {
	if (buffer.byteLength > MAX_IMAGE_BYTES) {
		throw new Error(`look_at image exceeds max size (${buffer.byteLength} > ${MAX_IMAGE_BYTES} bytes).`);
	}
}
...
async function prepareFileImage(filePath: string, ctx: ExtensionContext): Promise<PreparedImage> {
...
function decodeBase64Image(imageData: string, fallbackMimeType?: string): PreparedImage {
	...
	if (buffer.toString("base64").replace(/=+$/, "") !== base64.replace(/=+$/, "")) {
		throw new Error("look_at image_data must be valid base64 or a base64 data URI.");
	}
	assertByteLimit(buffer);
	return {
		type: "image",
...
async function prepareImage(params: NormalizedLookAtParams, ctx: ExtensionContext): Promise<PreparedImage> {
...
async function runVisionInspection(ctx: ExtensionContext, image: ImageContent, goal: string, signal?: AbortSignal) {
	...
	const loader = new DefaultResourceLoader({
		...
		noThemes: true,
		noContextFiles: true,
		systemPromptOverride: () => "You inspect image attachments and answer with concise factual text only.",
	...
	try {
		...
		await session.prompt(buildPrompt(goal), { images: [image], expandPromptTemplates: false });
		const text = collector.getText().trim() || getLastAssistantText(session).trim();
...
export default function multimodalLook(pi: ExtensionAPI): void {
	pi.registerTool({
		...
		async execute(_toolCallId: string, params: unknown, signal: AbortSignal | undefined, _onUpdate: unknown, ctx: ExtensionContext) {
			...
			const result = await runVisionInspection(ctx, image, normalized.goal, signal);
			return {
