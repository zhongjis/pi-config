...
Worktree isolation, scheduling (`schedule.ts`, `schedule-store.ts`, `croner`), agent mentions, and model-scope features are removed or dropped from this fork. Upstream execution and model-facing content are otherwise preserved. FleetView and Thinking Steps remain unchanged.
