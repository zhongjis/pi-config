...
describe("readonly-bash validator", () => {
  ...
  it.each([
    ...
    "gh search repos readonly bash",
    "gh search code readonly_bash --owner zshen",
    "gh search issues validation --repo owner/repo",
    "gh search prs validation --repo owner/repo",
  ...
  it.each([
    ...
    ["gh", "gh requires an explicit read-only subcommand"],
    ["gh auth status", "gh auth is not allowed"],
    ["gh api repos/owner/repo", "gh api is not allowed"],
    ["gh search commits fix", "gh search commits is outside the read-only allowlist"],
    ["gh search repos readonly --web", "gh browser-opening options are not allowed"],
    ["gh repo clone owner/repo", "gh repo clone is not allowed"],
    ["gh repo edit owner/repo", "gh repo edit is not allowed"],
    ["gh repo view owner/repo --web", "gh browser-opening options are not allowed"],
    ["gh issue create --title x", "gh issue create is not allowed"],
    ["gh issue edit 123", "gh issue edit is not allowed"],
    ["gh issue transfer 123", "gh issue transfer is not allowed"],
    ["gh workflow run test.yml", "gh workflow is not allowed"],
