...
it("leaves v1 dynamic children unclassified without authoritative ownership", async () => {
  ...
  const task = createWorkflowTask({ id: "v1", script: "" });
...
it("publishes identical transient nested v2 metadata on live and settled restore paths", async () => {
  ...
  const live = collapse(task.workflowProgress).agents;
  ...
  const restoredTask = createWorkflowTask({ id: "restored", script: "" });
  ...
  const rows = collapse(restoredTask.workflowProgress).agents;
