...
## Item 2 RESOLVED — unified root cause was the canonical guard, not target lookup

Corrected root cause (the earlier `clearCompleted`/lookup hypothesis above was WRONG). Instrumented the failing integration test (probes reverted): `[p-enter] -> [p-prepared] -> [p-capture-threw] "Persisted session directory is not canonical" -> [resume-lookup] durable=false`. So `captureResumeTarget` (`tools/agent.ts:725`) → `validatePersistedChildSession:342` → `confinedPath:315` THREW on macOS's symlinked tmpdir (`/var/folders` ≠ realpath `/private/var/folders`); the error was silently swallowed at `tools/agent.ts:750`; the durable resume-target was never recorded → resume-after-cleanup `target_unknown`. The **33 unit failures were the SAME `confinedPath:315` check firing directly**. chengfeng's microtask-timing theory was a red herring.

Fix (taishang security-reviewed — PRODUCT bug, Fix B): in `confinedPath`, dropped the `:315` self-canonical assertion and the `file !== target.sessionFile` clause at `:324`; kept the realpath containment guard (`dirname(realpath(file)) === realpath(dir)` + `rel` checks) which still rejects symlink escapes. Also surfaced the previously-silent background persist failure at `tools/agent.ts:750` via `pandaWarn("subagent.recovery.persist-failed", ...)`.

Verified: subagent unit 33 failed → **621 passed / 0 failed**; integration restoration 1 failed → **4 passed** (`target_unknown` → `restored_session`); security regression `rejects path escape, symlink escape, nested file` still GREEN; other subagent integration 8 passed; full unit project 2149 passed with the only failure being pre-existing `extensions/ulw` (commit `0441581`, unrelated); tsc 0 new errors. Files: `extensions/subagent/src/session-restoration.ts`, `extensions/subagent/src/tools/agent.ts`.
