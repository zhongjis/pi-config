...
export function buildPlanExecutionGoal(planPath: string): string {
  return [
    ...
    "- Batch-create all tasks as pending and wire their dependency graph.",
    "- Treat waves as labels; derive runnable work from the dependency graph.",
    "- Treat PLAN as the durable source of truth and Task as its synchronized runtime mirror: pending -> in_progress -> parent verified -> completed plus PLAN checked.",
    "- For each runnable set, launch independent implementation tasks as multiple foreground Agent calls in one assistant response; they execute concurrently while you block until all return.",
    "- Reserve background Agent calls for non-blocking exploration or research, never implementation parallelism.",
    ...
    "- All workers read only relevant shared notepad entries.",
    "- Mutation-capable workers append only relevant findings to the appropriate notepad and preserve unrelated entries.",
    "- Read-only researchers return findings to the parent for curation.",
    "- Parent rereads relevant notes and independently verifies them; notepad entries remain worker claims until verification.",
    "- Shared Agent-tree storage is same-user collaboration, not sandbox or security isolation.",
    "- Delegate every product-code, test-file, documentation, and git mutation; parent retains verification plus PLAN, Task, and notepad orchestration-state mutations.",
    "- Every worker prompt contains exactly six top-level sections, with capability-aware shared-note instructions under CONTEXT.",
    ...
    "- Final owners are fixed: F1 taishang; F2 parent code-quality gate; F3 parent manual QA; F4 direnjie.",
    "- On rejection, leave its Task in_progress and PLAN row unchecked, repair the responsible implementation workstream, then rerun every invalidated gate.",
    "- Surface all four approvals, then wait for the user's explicit okay before declaring complete.",
    "- Resume salvageable Agent sessions. Start fresh only when the prior session is unavailable or unsalvageable, and include its failure context; consult taishang before attempt 3.",
